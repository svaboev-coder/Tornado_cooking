...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'client-secret-key-here'
...
class RegistrationForm(FlaskForm):
    ...
    def validate_check_out_date(self, field):
        """Проверка, что дата отъезда не в прошлом и не раньше даты заезда"""
        if field.data:
            if field.data < datetime.now().date():
                raise ValidationError('Дата отъезда не может быть в прошлом')
...
def get_rooms(building):
    ...
    try:
        logger.info(f"Запрос номеров для корпуса: {building}")
        rooms = get_rooms_in_building(building)
        logger.info(f"Найдено номеров: {len(rooms)}")
...
def get_available_buildings():
    ...
    try:
        ...
        logger.info(f"Найдено корпусов: {len(buildings)} - {buildings}")
        return buildings
    except Exception as e:
...
if __name__ == '__main__':
    try:
        ...
        print("🌐 Клиентский интерфейс доступен по адресу: http://localhost:8080")
        
        # Запуск клиентского приложения на порту 8080
        app.run(host='0.0.0.0', port=8080, debug=False)
        
    except Exception as e:
        logger.error(f"Ошибка запуска клиентского приложения: {e}")
